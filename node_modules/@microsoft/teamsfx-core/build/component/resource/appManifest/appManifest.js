...
const appStudio_2 = require("./appStudio");
...
let AppManifest = class AppManifest {
    ...
    async addCapability(inputs, capabilities, isM365 = false) {
        return utils_2.manifestUtils.addCapabilities(inputs, capabilities, isM365);
    }
    async updateCapability(inputs, capability) {
        return utils_2.manifestUtils.updateCapability(inputs.projectPath, capability);
    }
    async deleteCapability(inputs, capability) {
        return utils_2.manifestUtils.deleteCapability(inputs.projectPath, capability);
    }
    async capabilityExceedLimit(inputs, capability) {
        return utils_2.manifestUtils.capabilityExceedLimit(inputs.projectPath, capability);
    }
    async provision(ctx, inputs, actionContext) {
        ...
        const res = await appStudio_2.createTeamsApp(ctx, inputs, ctx.envInfo, ctx.tokenProvider);
        if (res.isErr())
    ...
    async provisionForCLI(ctx, inputs, envInfo, tokenProvider, actionContext) {
        var _a;
        await ((_a = actionContext === null || actionContext === void 0 ? void 0 : actionContext.progressBar) === null || _a === void 0 ? void 0 : _a.next(localizeUtils_1.getLocalizedString("plugins.appstudio.provisionProgress", ctx.projectSetting.appName)));
        const res = await appStudio_2.createTeamsApp(ctx, inputs, envInfo, tokenProvider);
        return res;
    }
    async configure(ctx, inputs, actionContext) {
        ...
        const res = await appStudio_2.updateTeamsApp(ctx, inputs, ctx.envInfo, ctx.tokenProvider);
        if (res.isErr())
            return teamsfx_api_1.err(res.error);
        return teamsfx_api_1.ok(undefined);
    }
    async publish(ctx, inputs, actionCtx) {
        ...
        if (inputs.platform === teamsfx_api_1.Platform.VSCode &&
            ...
            try {
                const appPackagePath = await appStudio_2.buildTeamsAppPackage(ctx.projectSetting, inputs.projectPath, ctx.envInfo, false, actionCtx.telemetryProps);
        ...
        try {
            const res = await appStudio_2.publishTeamsApp(ctx, inputs, ctx.envInfo, ctx.tokenProvider.m365TokenProvider);
            if (res.isErr())
                return teamsfx_api_1.err(res.error);
    ...
    async validate(context, inputs) {
        var _a;
        const manifestRes = await utils_2.manifestUtils.getManifest(inputs.projectPath, context.envInfo, false);
        if (manifestRes.isErr()) {
            return teamsfx_api_1.err(manifestRes.error);
        }
        const manifest = manifestRes.value;
        const validationResult = await appStudio_2.validateManifest(manifest);
    ...
    async build(context, inputs) {
        var _a;
        const res = await appStudio_2.buildTeamsAppPackage(context.projectSetting, inputs.projectPath, context.envInfo);
        if (res.isOk()) {
    ...
    async deploy(context, inputs) {
        ...
        return await appStudio_2.updateManifest(context, inputs);
    }
    /**
    ...
    async getTeamsAppId(ctx, inputs, envInfo) {
        ...
        const manifestResult = await utils_2.manifestUtils.getManifest(inputs.projectPath, envInfo, false);
