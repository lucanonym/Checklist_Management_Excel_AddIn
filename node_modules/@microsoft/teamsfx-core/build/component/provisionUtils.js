...
class ProvisionUtils {
    async preProvision(context, inputs) {
        ...
        if (!isLocalDebug) {
            ...
            const res = await utils_2.checkWhetherLocalDebugM365TenantMatches(envInfo, ctx.telemetryReporter, utils_3.isCSharpProject(ctx.projectSetting.programmingLanguage), tenantIdInConfig, ctx.tokenProvider.m365TokenProvider, inputs.projectPath);
            if (res.isErr()) {
        ...
        if (projectSettingsHelperV3_1.hasAzureResourceV3(ctx.projectSetting) && envInfo.envName !== "local") {
            ...
            if (solutionConfigRes.value.hasSwitchedSubscription || hasSwitchedM365Tenant) {
                const handleConfigFilesWhenSwitchAccountsRes = await util_1.handleConfigFilesWhenSwitchAccount(envInfo, ctx.projectSetting.appName, inputs.projectPath, hasSwitchedM365Tenant, solutionConfigRes.value.hasSwitchedSubscription, hasBotServiceCreatedBefore, utils_3.isCSharpProject(ctx.projectSetting.programmingLanguage));
            ...
            const handleConfigFilesWhenSwitchAccountsRes = await util_1.handleConfigFilesWhenSwitchAccount(envInfo, ctx.projectSetting.appName, inputs.projectPath, hasSwitchedM365Tenant, false, false, utils_3.isCSharpProject(ctx.projectSetting.programmingLanguage));
            if (handleConfigFilesWhenSwitchAccountsRes.isErr()) {
                addShouldSkipWriteEnvInfo(handleConfigFilesWhenSwitchAccountsRes.error);
                return teamsfx_api_1.err(handleConfigFilesWhenSwitchAccountsRes.error);
            }
