...
async function scaffoldSPFx(context, inputs, outputFolderPath) {
    ...
    try {
        const isAddSpfx = inputs.stage === teamsfx_api_1.Stage.addFeature && common_1.isSPFxMultiTabEnabled();
        ...
        if (!isAddSpfx) {
            solutionName =
                ((_b = context.projectSetting) === null || _b === void 0 ? void 0 : _b.appName) ||
                    ((_c = context.projectSettings) === null || _c === void 0 ? void 0 : _c.appName);
        }
        else {
            const yorcPath = path.join(inputs.projectPath, "SPFx", ".yo-rc.json");
            if (!(await fs_extra_1.default.pathExists(yorcPath))) {
                yoPersisted = false;
                await fs_extra_1.default.ensureFile(yorcPath);
                await fs_extra_1.default.writeJSON(yorcPath, {
                    "@microsoft/generator-sharepoint": {
                        solutionName: ((_d = context.projectSetting) === null || _d === void 0 ? void 0 : _d.appName) ||
                            ((_e = context.projectSettings) === null || _e === void 0 ? void 0 : _e.appName),
                    },
                });
            }
        }
        ...
        yoEnv.PATH = tools_1.isYoCheckerEnabled()
            ? `${await (await yoChecker.getBinFolders()).join(path.delimiter)}${path.delimiter}${(_f = process.env.PATH) !== null && _f !== void 0 ? _f : ""}`
            : process.env.PATH;
        const args = [
        ...
        if (error.name === "DependencyInstallFailed") {
            ...
            (_g = context.logProvider) === null || _g === void 0 ? void 0 : _g.error(`We've encountered some issues when trying to install prerequisites under HOME/.fx folder.  Learn how to remediate by going to this link(aka.ms/teamsfx-spfx-help) and following the steps applicable to your system: \n ${yoMessage} \n ${generatorMessage}`);
            (_h = context.logProvider) === null || _h === void 0 ? void 0 : _h.error(`Teams Toolkit recommends using ${yoInfo.displayName} ${genInfo.displayName}`);
        }
        if (error.message &&
            error.message.includes("'yo' is not recognized as an internal or external command")) {
            (_j = context.logProvider) === null || _j === void 0 ? void 0 : _j.error("NPM v6.x with Node.js v12.13.0+ (Erbium) or Node.js v14.15.0+ (Fermium) is recommended for spfx scaffolding and later development. You can use correct version and try again.");
        }
