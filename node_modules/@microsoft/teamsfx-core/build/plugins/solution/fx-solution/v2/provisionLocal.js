...
async function provisionLocalResource(ctx, inputs, localSettings, tokenProvider, envInfo) {
    ...
    const azureSolutionSettings = utils_2.getAzureSolutionSettings(ctx);
    if (utils_2.isAzureProject(azureSolutionSettings)) {
        if (ctx.permissionRequestProvider === undefined) {
            ctx.permissionRequestProvider = new permissionRequest_1.PermissionRequestFileProvider(inputs.projectPath);
        }
        const result = await utils_2.ensurePermissionRequest(azureSolutionSettings, ctx.permissionRequestProvider);
        if (result.isErr()) {
    ...
    const m365TenantMatches = await utils_2.checkWhetherLocalDebugM365TenantMatches(envInfo, ctx.telemetryReporter, utils_1.isCSharpProject(ctx.projectSetting.programmingLanguage), localDebugTenantId, tokenProvider.m365TokenProvider, inputs.projectPath);
    if (m365TenantMatches.isErr()) {
        return new teamsfx_api_1.v2.FxFailure(m365TenantMatches.error);
    }
    const plugins = utils_2.getSelectedPlugins(ctx.projectSetting);
    ...
    const parseTenantIdresult = utils_2.loadTeamsAppTenantIdForLocal(localSettings, appStudioTokenJson, envInfo);
