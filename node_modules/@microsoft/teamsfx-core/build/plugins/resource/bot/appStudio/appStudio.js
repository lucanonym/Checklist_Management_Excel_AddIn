...
class AppStudio {
    ...
    static async getBotRegistration(accessToken, botId) {
        const axiosInstance = AppStudio.newAxiosInstance(accessToken);
    ...
    static async updateMessageEndpoint(accessToken, botId, endpoint) {
        const axiosInstance = AppStudio.newAxiosInstance(accessToken);
        ...
        let response = undefined;
        try {
            response = await retryHandler_1.RetryHandler.Retry(() => axiosInstance.post(`${AppStudio.baseUrl}/api/botframework/${botId}`, botReg));
        }
        catch (e) {
...
AppStudio.baseUrl = constants_1.getAppStudioEndpoint();
