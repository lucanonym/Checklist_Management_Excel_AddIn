...
class FxCore {
    ...
    async executeUserTaskV3(func, inputs, ctx) {
        ...
        if (func.method === "addCICDWorkflows") {
            ...
            inputs.stage = teamsfx_api_1.Stage.addFeature;
            inputs[question_1.AzureSolutionQuestionNames.Features] = question_1.SingleSignOnOptionItem.id;
    ...
    async getProjectConfig(inputs, ctx) {
        ...
        let envState = (_a = ctx.solutionContext) === null || _a === void 0 ? void 0 : _a.envInfo.state;
        if (envState)
            envState = migrate_1.convertEnvStateMapV3ToV2(envState);
    ...
    async _init(inputs, ctx, isInitExistingApp = false) {
        ...
        await fs.ensureDir(path.join(projectPath, `.${teamsfx_api_1.ConfigFolderName}`));
        await fs.ensureDir(path.join(await utils_2.getProjectTemplatesFolderPath(projectPath), `${teamsfx_api_1.AppPackageFolderName}`));
